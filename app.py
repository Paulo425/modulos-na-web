...
@app.get("/download/azimute-az/log/<uuid>")
...
@app.get("/download/azimute-az/zip/<uuid>/<fname>")
...
def gerar_memorial_azimute_az():
    ...
    if request.method == 'POST':
        ...
        # Mantém o mesmo nome e semântica que você tinha:
        ...
        # Uploads salvos com nome único e depois copiados/consumidos pelo pipeline
        arquivo_excel = request.files['excel']
        arquivo_dxf = request.files['dxf']
        caminho_excel = salvar_com_nome_unico(arquivo_excel, app.config['UPLOAD_FOLDER'])
        caminho_dxf   = salvar_com_nome_unico(arquivo_dxf,   app.config['UPLOAD_FOLDER'])
        ...
        try:
            # Preferimos o padrão DECOPA: passar ID_EXECUCAO por env e parâmetros nomeados
            env = os.environ.copy()
            ...
            cmd = [
                ...
                "--excel", caminho_excel,
                "--dxf", caminho_dxf,
                "--sentido", sentido_poligonal,  # o main deve aceitar; se não aceitar, ajuste lá
            ]
            app.logger.info(f"[AZIMUTE_AZ] CMD: {cmd}")

            proc = Popen(cmd, stdout=PIPE, stderr=STDOUT, text=True, env=env)

            # Stream para arquivo de log + manter primeiras linhas para erro amigável
            log_lines = []
            with exec_log_path.open('w', encoding='utf-8') as lf:
                t0 = time.time()
                # 5 minutos para o processo (ajuste se precisar)
                timeout_s = 300
                while True:
                    if proc.poll() is not None:
                        # terminou; drena qualquer resto da stdout
            ...
            app.logger.info(f"[AZIMUTE_AZ] returncode={proc.returncode}")

            # Descobrir ZIP(s) desta execução
            manifest_path = Path(dir_concluido) / "RUN.json"
            zip_files = []

            # Pequena espera para evitar condição de corrida na escrita do RUN.json
            for _ in range(10):
            ...
            try:
                if manifest_path.exists():
                    ...
                    zip_files = [os.path.basename(z) for z in manifest.get("zip_files", []) if z]
                    app.logger.info(f"[AZIMUTE_AZ] RUN.json: {zip_files}")
            except Exception as e:
                app.logger.warning(f"[AZIMUTE_AZ] RUN.json inválido: {e}")

            # Fallback: listar *.zip em CONCLUIDO
            if not zip_files:
                zip_files = sorted([p.name for p in Path(dir_concluido).glob("*.zip")])

            # Montar URLs seguras (com UUID)
            if zip_files:
            ...
            app.logger.exception("[AZIMUTE_AZ] Falha ao executar pipeline")
            # registra erro no log de execução para o usuário baixar
            try:
        ...
        # Limpeza dos uploads temporários (equivalente ao finally)
        for p in (caminho_excel, caminho_dxf):
            try:
                if p and os.path.exists(p):
                    os.remove(p)
...
def download_log_angulo_az(uuid):
    if not re.fullmatch(r"[0-9a-fA-F]{8}", uuid):
        abort(400, "UUID inválido")
    dir_conc = Path(BASE_DIR) / "tmp" / uuid / "CONCLUIDO"
    if not dir_conc.exists():
...
@app.get("/download/angulo-az/zip/<uuid>/<fname>")
def download_zip_angulo_az(uuid, fname):
    if not re.fullmatch(r"[0-9a-fA-F]{8}", uuid):
        abort(400, "UUID inválido")
    if Path(fname).name != fname:
        abort(400, "Nome de arquivo inválido")
...
def gerar_memorial_angulo_az():
    ...
    if request.method == 'POST':
        cidade = request.form['cidade'].strip()
        # mesmo padrão do AZIMUTE_AZ
        sentido_poligonal = 'anti_horario' if 'sentidoPoligonal' in request.form else 'horario'
        app.logger.info(f"[ANGULO_AZ] sentidoPoligonal={request.form.get('sentidoPoligonal')} -> {sentido_poligonal}")
        ...
        try:
            ...
            app.logger.info(f"[ANGULO_AZ] CMD: {cmd}")

            proc = Popen(cmd, stdout=PIPE, stderr=STDOUT, text=True, env=env)

            # Stream do log + primeiras linhas para diagnóstico
            log_lines = []
            with exec_log_path.open('w', encoding='utf-8') as lf:
                t0 = time.time()
                timeout_s = 300
            ...
            proc_ok = (proc.returncode == 0)
            app.logger.info(f"[ANGULO_AZ] returncode={proc.returncode}")

            # Descobrir ZIP(s) desta execução
            manifest_path = Path(dir_concluido) / "RUN.json"
            zip_files = []

            # pequena espera para evitar corrida na escrita do RUN.json
            for _ in range(10):
                if manifest_path.exists():
                    break
                time.sleep(0.1)
            ...
            try:
                if manifest_path.exists():
                    manifest = json.loads(manifest_path.read_text(encoding='utf-8'))
                    zip_files = [os.path.basename(z) for z in manifest.get("zip_files", []) if z]
                    app.logger.info(f"[ANGULO_AZ] RUN.json: {zip_files}")
            except Exception as e:
                app.logger.warning(f"[ANGULO_AZ] RUN.json inválido: {e}")
            ...
            if zip_files:
                zip_urls = [url_for("download_zip_angulo_az", uuid=run_uuid, fname=f) for f in zip_files]
                zip_url = zip_urls[0]
                zip_download = zip_files[0]
                success = True
            ...
            app.logger.exception("[ANGULO_AZ] Falha ao executar pipeline")
            try:
                exec_log_path.write_text(f"Erro fatal: {type(e).__name__}: {e}", encoding="utf-8")
            except Exception:
                pass
        ...
        # Link para baixar o log desta execução
        log_url = url_for("download_log_angulo_az", uuid=run_uuid)

        return render_template(
            "formulario_ANGULO_AZ.html",
            resultado=resultado,
            erro=erro_execucao,
            success=success,
            zip_url=zip_url,
...
@app.get("/download/angulo-p1p2/log/<uuid>")
def download_log_angulo_p1_p2(uuid):
    if not re.fullmatch(r"[0-9a-fA-F]{8}", uuid):
        abort(400, "UUID inválido")
    dir_conc = Path(BASE_DIR) / "tmp" / uuid / "CONCLUIDO"
    if not dir_conc.exists():
...
@app.get("/download/angulo-p1p2/zip/<uuid>/<fname>")
def download_zip_angulo_p1_p2(uuid, fname):
    if not re.fullmatch(r"[0-9a-fA-F]{8}", uuid):
        abort(400, "UUID inválido")
    if Path(fname).name != fname:
        abort(400, "Nome de arquivo inválido")
...
def gerar_memorial_angulo_p1_p2():
    ...
    if request.method == 'POST':
        cidade = request.form['cidade'].strip()
        sentido_poligonal = 'anti_horario' if 'sentidoPoligonal' in request.form else 'horario'
        app.logger.info(f"[ANGULO_P1_P2] sentidoPoligonal={request.form.get('sentidoPoligonal')} -> {sentido_poligonal}")
        ...
        try:
            ...
            app.logger.info(f"[ANGULO_P1_P2] CMD: {cmd}")

            proc = Popen(cmd, stdout=PIPE, stderr=STDOUT, text=True, env=env)

            # Stream p/ arquivo + primeiras linhas para erro amigável
            log_lines = []
            with exec_log_path.open('w', encoding='utf-8') as lf:
                t0 = time.time()
                timeout_s = 300
            ...
            proc_ok = (proc.returncode == 0)
            app.logger.info(f"[ANGULO_P1_P2] returncode={proc.returncode}")
            ...
            try:
                if manifest_path.exists():
                    manifest = json.loads(manifest_path.read_text(encoding='utf-8'))
                    zip_files = [os.path.basename(z) for z in manifest.get("zip_files", []) if z]
                    app.logger.info(f"[ANGULO_P1_P2] RUN.json: {zip_files}")
            except Exception as e:
                app.logger.warning(f"[ANGULO_P1_P2] RUN.json inválido: {e}")
            ...
            if zip_files:
                zip_urls = [url_for("download_zip_angulo_p1_p2", uuid=run_uuid, fname=f) for f in zip_files]
                zip_url = zip_urls[0]
                zip_download = zip_files[0]
                success = True
            ...
            app.logger.exception("[ANGULO_P1_P2] Falha ao executar pipeline")
            try:
                exec_log_path.write_text(f"Erro fatal: {type(e).__name__}: {e}", encoding="utf-8")
            except Exception:
                pass
        ...
        # Link para baixar o log desta execução
        log_url = url_for("download_log_angulo_p1_p2", uuid=run_uuid)

        return render_template(
            "formulario_angulo_p1_p2.html",
            resultado=resultado,
            erro=erro_execucao,
            success=success,
            zip_url=zip_url,
