...
def create_memorial_descritivo(
    ...
    if not lines:
        logger.error("❌ Nenhuma linha disponível para criar o memorial descritivo.")
        return None
...
def validar_arquivos_gerados(uuid_str, tipo, matricula, diretorio_concluido):
    arquivos_esperados = {
        "XLSX_FECHADA": f"{uuid_str}_FECHADA_{tipo}_{matricula}.xlsx",
        "XLSX_ABERTA":  f"{uuid_str}_ABERTA_{tipo}_{matricula}.xlsx",
        "DOCX":         f"{uuid_str}_FECHADA_{tipo}_{matricula}.docx",
        "DXF":          f"{uuid_str}_FECHADA_{tipo}_{matricula}.dxf"
    }

    logger.info("🔎 Verificando se todos os arquivos esperados foram gerados...")

    for label, nome_arquivo in arquivos_esperados.items():
        caminho = os.path.join(diretorio_concluido, nome_arquivo)
        if os.path.exists(caminho):
            logger.info(f"✅ {label}: {nome_arquivo} localizado com sucesso.")
        else:
            logger.warning(f"❌ {label}: {nome_arquivo} NÃO encontrado em {diretorio_concluido}.")
...
def main_poligonal_fechada(uuid_str, excel_path, dxf_path, diretorio_preparado, diretorio_concluido, caminho_template):
    ...
    if doc and lines:
        ...
        if excel_file_path:
            ...
            validar_arquivos_gerados(uuid_str, tipo, matricula, diretorio_concluido)
        else:
