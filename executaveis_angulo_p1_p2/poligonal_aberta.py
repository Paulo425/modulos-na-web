...
def associar_pontos_rotulos(points, texts):
    pontos_rotulados = []
    for txt_label, txt_coord in texts.items():
        # Encontrar ponto mais próximo
        ponto_mais_proximo = min(points, key=lambda p: calculate_distance(txt_coord, p))
        pontos_rotulados.append((txt_label, ponto_mais_proximo))
        points.remove(ponto_mais_proximo)  # remover para não reutilizar
    # Ordenar pelos rótulos P1, P2, etc.
    pontos_rotulados.sort(key=lambda x: int(x[0][1:]))  
    return pontos_rotulados
